...
describe("deterministicPartitionKey", () => {
  ...
  it("Returns the literal '0' when given no parameters", () => {
    const trivialKey = deterministicPartitionKey();
    expect(trivialKey).toBe("0");
  });

  it("Returns a hash when given an empty object as parameter", () => {
    const trivialKey = deterministicPartitionKey({});
    expect(trivialKey).toBe(generatedHashForEmptyObject);
  });

  it("Returns a hash when given an empty array as parameter", () => {
    const trivialKey = deterministicPartitionKey([]);
    expect(trivialKey).toBe(generatedHashForEmptyArray);
  });

  it("Returns a hash when given a 'new Object()' as parameter", () => {
    const trivialKey = deterministicPartitionKey(new Object());
    expect(trivialKey).toBe(generatedHashForNewObject);
  });

  it("Returns a hash when given an object with a undefined 'partitionKey' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: undefined });
    expect(trivialKey).toBe(generatedHashForEmptyObject);
  });

  it("Returns a hash when given an object with a null 'partitionKey' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: null });
    expect(trivialKey).toBe(generatedHashForNullPartitionKey);
  });

  it("Returns a hash when given an object with 'partitionKey: 0' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: 0 });
    expect(trivialKey).toBe(generatedHashForZeroPartitionKey);
  });

  it("Returns a hash when given an object with partitionKey: '' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: "" });
    expect(trivialKey).toBe(generatedHashForEmptyStringPartitionKey);
  });

  it("Returns a hash when given an object with 'partitionKey: false' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: false });
    expect(trivialKey).toBe(generatedHashForFalsePartitionKey);
  });

  it("Returns a hash when given an object with 'partitionKey: NaN' key as parameter", () => {
    const trivialKey = deterministicPartitionKey({ partitionKey: NaN });
    expect(trivialKey).toBe(generatedHashForNaNPartitionKey);
  });

  it("Returns the '0' when given an object with 'partitionKey: '0' as parameter", () => {
    const partitionKey = "0";
    const trivialKey = deterministicPartitionKey({ partitionKey });
    expect(trivialKey).toBe(partitionKey);
  });

  it("Returns a stringfied number when given an object with a 'partitionKey' key containing a number", () => {
    const partitionKey = 123;
    const trivialKey = deterministicPartitionKey({ partitionKey });
    expect(trivialKey).toBe(String(partitionKey));
  });

  it("Returns 'null' when given an object with a 'partitionKey: Infinity' as parameter", () => {
    const partitionKey = Infinity;
    const trivialKey = deterministicPartitionKey({ partitionKey });
    expect(trivialKey).toBe("null");
  });

  it("Returns a stringfied number when given an object with a 'partitionKey' key containing a big number", () => {
